...
@Bean CommandLineRunner seed(OperatingUnitRepository units,WorkRecordRepository records,ResourceRegisterRepository resources,ReviewRecordRepository reviews,UserRepository users,PasswordEncoder encoder){return args->{if(units.count()>0)return;
 ...
 reviews.saveAll(List.of(new ReviewRecord("RCA-260801-032",a,"根因准确率",12,1,ReviewRecord.Result.PENDING,"沈策"),new ReviewRecord("RCA-260801-011",b,"处置有效性",8,0,ReviewRecord.Result.PASSED,"周屿"),new ReviewRecord("RCA-260731-018",c,"漏报风险",16,3,ReviewRecord.Result.FAILED,"沈策")));
 String demo=encoder.encode("Demo@2026"); users.saveAll(List.of(new UserAccount("operator",demo,"周屿",UserAccount.Role.DOMAIN_USER,"OPS-TRADE"),new UserAccount("planner",demo,"沈策",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"宋惟",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}}
